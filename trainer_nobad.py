    ...
    for game in range(10000):
        ...
        for i in range(actual_batch_size):
            if sample_batch[i][3] is None:
                ...
                qsa_batch[i, action_batch[i]] = sample_batch[i][2] + gamma*get_max_reward(next_state_batch[i].reshape(1, 9), model, sess)

        model.train_batch(state_batch, qsa_batch, sess)

        epsilon = 0.9*np.exp(-0.0001*game)
    model.save(sess, 'tic_tac_toe_model_nobad')
    model.plot_losses('losses.png')
